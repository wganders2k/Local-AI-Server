...
Mimic personas are defined as separate `[mimic_<member>]` sections in `models.ini`, all pointing to the same GGUF. The system prompt is injected per-request by the Discord bot (see `discord-bot.md` §7.1).
...
> **System prompts:** All system prompts are injected per-request by the Discord bot (see `discord-bot.md` §7). They are not baked into `models.ini` — this allows per-persona customisation without restarting the server.
