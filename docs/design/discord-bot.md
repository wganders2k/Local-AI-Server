...
**Scope:** design parameters for the Discord bot component. For system architecture (hardware, GPU arbitration, proxy, RAG pipeline), see [`system.md`](system.md).
...
The proxy injects nothing (`system.md` §6.3). Every system prompt originates here.
...
All three accept optional `start_date` / `end_date` in ISO 8601, which the RAG service converts to epoch comparisons against chunk metadata (`system.md` §9.1).
...
*For system architecture, GPU arbitration, and the RAG pipeline, see [`system.md`](system.md).*
